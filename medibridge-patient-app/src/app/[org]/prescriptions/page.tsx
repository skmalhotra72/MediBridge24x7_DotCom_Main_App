...
const formatPatientInfo = (prescription: PrescriptionItem): string => {
  const parts: string[] = [];
...
// Helper function to format doctor info
const formatDoctorInfo = (prescription: PrescriptionItem): string | null => {
  const parts: string[] = [];
...
export default async function PrescriptionsPage({ params }: PrescriptionsPageProps) {
  ...
  // Enhanced query with all patient and doctor fields
  ...
  const analyzedCount = prescriptions?.filter(p => p.status === 'analyzed' || p.status === 'completed').length || 0;
  const pendingCount = prescriptions?.filter(p => p.status === 'pending' || p.status === 'processing').length || 0;
              ...
              >
                <div className="flex items-start justify-between gap-4">
                  ...
                  <div className="flex items-start gap-5 flex-1">
                    {/* Document Type Icon */}
                    <div className={`w-14 h-14 rounded-2xl flex items-center justify-center shadow-lg group-hover:scale-110 group-hover:rotate-3 transition-all duration-500 flex-shrink-0 ${
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex items-start gap-3 mb-2">
                        <h3 className="text-sm font-semibold text-white group-hover:text-cyan-300 transition-colors line-clamp-2 flex-1 min-w-0">
                          {prescription.user_question || prescription.chief_complaint || prescription.diagnosis || `Prescription #${prescription.id.slice(0, 8)}`}
                        </h3>
                        <span className={`px-2.5 py-1 text-xs font-bold rounded-full flex-shrink-0 whitespace-nowrap ${
                          ...
                          {prescription.status === 'analyzed' || prescription.status === 'completed' ? '✓ Analyzed' : 
                           prescription.status === 'pending' || prescription.status === 'processing' ? '⏳ Processing' : 
                           'New'}
                        </span>
                      </div>

                      {/* ============================================ */}
                      {/* NEW: Patient & Doctor Info Badges */}
                      {/* ============================================ */}
                      <div className="flex flex-wrap items-center gap-2 mb-3">
                        {/* Patient Badge - Name, Age, Gender */}
                        <div className="inline-flex items-center gap-1.5 bg-cyan-500/10 border border-cyan-500/20 px-2.5 py-1 rounded-lg">
                        ...
                        <div className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-lg ${
                          ...
                          {prescription.document_type === 'lab_report' ? (
                            <>
                              ...
                              <span className="text-purple-300 text-xs font-medium">Lab Report</span>
                            </>
                          ) : (
                            <>
                              ...
                              <span className="text-blue-300 text-xs font-medium">Prescription</span>
                            </>
                          )}
                        </div>
                      </div>

                      {/* Bottom Row: Date, Time, Medicines, Tests */}
                      <div className="flex flex-wrap items-center gap-4 text-sm text-gray-400">
                        ...
                        {/* Medicines Count */}
                        {prescription.total_medicines && prescription.total_medicines > 0 && (
                          <>
                            <span className="text-gray-600">•</span>
                            <span className="flex items-center gap-1.5 text-blue-400">
                              ...
                              {prescription.total_medicines} medicines
                            </span>
                          </>
                        )}
                        
                        {/* Tests Count */}
                        {prescription.total_tests && prescription.total_tests > 0 && (
                          <>
                            <span className="text-gray-600">•</span>
                            <span className="flex items-center gap-1.5 text-purple-400">
                              ...
                              {prescription.total_tests} tests
                            </span>
                          </>
                        )}
                      </div>
                  ...
                  <div className="flex items-center gap-3 flex-shrink-0">
                    <Link
                      ...
                      className="px-5 py-2.5 bg-gradient-to-r from-cyan-500 to-blue-500 text-white font-semibold rounded-xl hover:from-cyan-600 hover:to-blue-600 shadow-lg hover:shadow-cyan-500/50 transition-all duration-300 hover:scale-105 flex items-center gap-2"
                    >
